...
class Lambda_Helper: 
    ...
    def __init__(self, region : str, lambda_layer_arn : str, lambda_execution_role : str):
        ...
        self.role_arn = lambda_execution_role #f"arn:aws:iam::{account_id}:role/LambdaRoleDLAICourse"
    ...
    def deploy_function(self, code_file_names, function_name):
        ...
        try:
            ...
            print(f"Function {function_name} code updated: {response['LastModified']}")
            ...
            response = self.lambda_client.create_function(
                ...
                Layers=[self.lambda_layer_arn],  #[os.environ['LAMBDALAYERVERSIONARN']],
                Timeout=120,
                Code={
    ...
    def add_lambda_trigger(self, bucket_name, filter_rule_list,function_name="") :
        ...
        try:
            lambda_arn = self.lambda_client.get_function(FunctionName=function_name)['Configuration']['FunctionArn']  #get Function ARN
            print(f"Function ARN : {lambda_arn}")
            
            policy = self.lambda_client.get_policy(FunctionName=function_name)['Policy']
            policy_dict = json.loads(policy)
        
            for statement in policy_dict['Statement']:
                if statement['Action'] == 'lambda:InvokeFunction' and lambda_arn in statement['Resource']:
        ...
        try:
            response = self.lambda_client.add_permission(
                ...
                StatementId=f"s3-trigger-permission-for-{function_name}",  # A unique statement ID
        ...
        try:
            print(f"Bucket name: {bucket_name} and notification configuration: {notification_configuration} ")
